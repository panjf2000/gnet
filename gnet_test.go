...
func (s *testServer) OnTraffic(c Conn) (action Action) {
	...
	assert.EqualValues(s.tester, len(buf), n, "short writev")
...
func closeTestServers(t *testing.T, servers []io.Closer) {
	...
	for _, server := range servers {
		if err := server.Close(); err != nil && !errors.Is(err, net.ErrClosed) {
			t.Errorf("Close error on %v: %v", server, err)
		}
	}
}
...
func (p *udpProxyServer) OnTraffic(c Conn) Action {
	if c.LocalAddr().String() == p.ListenerAddr { // it's a server connection
		...
		assert.EqualValuesf(p.tester, len(buf), n, "Write should send %d bytes, but sent %d bytes", len(buf), n)
